...
namespace DAL
{
    public class DbContextFactory : IDbContextFactory, IDisposable
    {
        ...
        {
            var options = new DbContextOptionsBuilder<GAFDbContext>().UseNpgsql(settings.Value.DbConnectionString,
                npgsqlOptionsAction: s =>
                {
                  s.EnableRetryOnFailure(maxRetryCount: 10, maxRetryDelay: TimeSpan.FromSeconds(30), errorCodesToAdd: null);
                }).Options;
            DbContext = new GAFDbContext(options);
        ...
        public GAFDbContext DbContext { get; private set; }
        /// <summary>
        /// Release DB context
        /// </summary>
        public void Dispose()
